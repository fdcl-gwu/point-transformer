...
def test():
    ...
    # Load saved model
    checkpoint_path = "/home/karlsimon/point-transformer/log/pose_estimation/2025-02-17_12-45/best_model.pth"
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(test_dl):
            ...
            loss = pose_criterion(pred_t, gt_translation).item()  # Computed for the batch
            ...
            batch_file_names = [dataset.data_paths[test_ds.indices[i]][0] for i in range(batch_start_idx, batch_start_idx + len(gt_translation))]

            # Store inference results in a structured format
            for i in range(len(gt_translation)):
                result_data.append({
